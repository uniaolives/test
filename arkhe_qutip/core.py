...
class ArkheSolver:
    ...
    def solve(self, rho0: Union[qt.Qobj, ArkheQobj], tlist: np.ndarray,
        ...
        if self.phi_coupling > 0:
            # Physically meaningful coupling: alpha * phi * M
            # where M is a diagonal operator that affects relative phases
            # H_phi = alpha * phi * sigma_z (for each qubit)
            dim = self.H.dims[0]
            if len(dim) == 1: # Single system
                M = qt.sigmaz()
            else: # Multi-partite
                M = sum([qt.tensor([qt.sigmaz() if i==j else qt.identity(d) for j, d in enumerate(dim)]) for i in range(len(dim))])

            H_total = self.H + self.phi_coupling * M
