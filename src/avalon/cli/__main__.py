...
from ..analysis.coral_protocol import CoralProtocolSimulator
from ..analysis.recruitment import WhisperProtocol
...
def pdcp_calibrate(amazon_data: float = 4.5, days_to_sirius: float = 42.3):
    ...
    t = list(np.linspace(0, 10, 10))
    res = pdcp.run_rhythmic_cycle([amazon_data] * 10, t, 1.0)
    typer.echo(json.dumps(res, indent=2))
...
def pdcp_simulate_threshold():
    ...
    # Generate perfectly resonant stream for simulation success
    natural_stream = v0 * (1 + 0.05 * np.sin(2 * np.pi * f_phi * t))

    # Run cycle with alignment (days = 0)
    pdcp.days_to_alignment = 0.0
    # Increased sirius energy to reach LTP
    res = pdcp.run_rhythmic_cycle(natural_stream.tolist(), t.tolist(), 5.0)
...
@app.command()
def coral_simulate(variations: int = 1000):
    """
    Execute SRQ simulation for the Coral Protocol (IETD-Lambda v1.1).
    """
    sim = CoralProtocolSimulator()
    typer.echo(f"🌊 INITIATING CORAL SRQ SIMULATION ({variations} variations)...")

    # Show initial state
    status = sim.get_current_bleaching_status(0)
    typer.echo(f"   [!] Current Temp: {status['current_temp']}°C ({status['deviation']}°C deviation)")

    results = sim.run_srq(variations=variations)
    typer.echo(json.dumps(results, indent=2))

    # Calculate bleaching after 72h delay
    final_status = sim.get_current_bleaching_status(72)
    typer.echo(f"\n⚠️  Projected Bleaching if no action taken (72h): {final_status['total_bleaching_percent']:.2f}%")
    typer.echo(f"✨ Optimal HSP70 Upregulation: {results['optimal_syntax']['hsp_upregulation']:.2%}")

@app.command()
def recruitment_whisper(sample: int = 500):
    """
    Run the Whisper Protocol global scan for Water Cell candidates.
    """
    whisper = WhisperProtocol()
    typer.echo(f"👥 INITIATING GLOBAL WHISPER SCAN (Sample size: {sample})...")

    identified = whisper.perform_global_scan(population_sample=sample)
    typer.echo(f"   [+] Candidates identified: {len(identified)}")

    for c in identified[:3]: # Show a few
        typer.echo(f"   - {c.id}: Coherence={c.eeg_coherence:.4f}, Resonance={c.resonance_detected}")

    # Simulate activation of one candidate via Glass Door
    if identified:
        candidate_id = identified[0].id
        typer.echo(f"\n🚪 ACTIVATING GLASS DOOR FOR {candidate_id}...")
        decision = whisper.activate_glass_door(candidate_id)
        typer.echo(json.dumps(decision, indent=2))

    typer.echo("\n📊 Scan Summary:")
    typer.echo(json.dumps(whisper.get_summary(), indent=2))
