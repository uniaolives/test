...
async fn validate(client: web::Data<Client>, body: web::Bytes) -> impl Responder {
    ...
    let review_response = AdmissionReview::<DynamicObject> {
        types: admission_review.types.clone(),
        request: None,
...
async fn main() -> std::io::Result<()> {
    ...
    let certs_raw = rustls_pemfile::certs(&mut cert_data.as_slice())
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
    let mut keys_raw = rustls_pemfile::pkcs8_private_keys(&mut key_data.as_slice())
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;

    if keys_raw.is_empty() {
        return Err(std::io::Error::new(std::io::ErrorKind::Other, "No private key found"));
    }

    let certs = certs_raw.into_iter().map(rustls::Certificate).collect();
    let key = rustls::PrivateKey(keys_raw.remove(0));
