...
func TestValidatorExitMinslack(t *testing.T) {
	...
	// Simulate 10 simultaneous exits being assigned delays.
	// With maxChurn=10 and queueSize growing from 0 to 9:
	// - First exit: queueSize=0 → empty queue → MinValidatorWithdrawabilityDelay (256)
	// - Exits 1..4: queueSize < maxChurn/2 → MinValidatorWithdrawabilityDelay/2 (128)
	// - Exits 5..9: proportional delay
	// - At queueSize=10 (full): MaxSeedLookahead (4)
	...
	// Delays should generally decrease as queue fills (shorter delays for later exits).
	// At minimum, the last delay should be less than or equal to the first.
