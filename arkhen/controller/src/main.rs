use kube::{
    api::{Api, Patch, PatchParams, ResourceExt, EvictParams},
    client::Client,
...
async fn drain_node(client: Client, node_name: &str) -> Result<(), anyhow::Error> {
    ...
    if let Some(spec) = cordon_node.spec.as_mut() {
    ...
    let lp = kube::api::ListParams::default().fields(&format!("spec.nodeName={}", node_name));
    let pods = pods_api.list(&lp).await?;

    for pod in pods {
        let pod_name = pod.name_any();
        println!("Evicting pod {} from node {}", pod_name, node_name);
        let ep = EvictParams::default();
        let _ = pods_api.evict(&pod_name, &ep).await;
    }
