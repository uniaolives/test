...
echo "🤖 Installing AI commit hooks..."
...
if echo "$COMMIT_MSG" | grep -q "@autogen\|@reasoning\|@collab"; then
    echo "🔍 AI-generated commit detected. Validating context..."

    # Check for required sections in body
    BODY=$(tail -n +2 "$COMMIT_MSG_FILE")
    if ! echo "$BODY" | grep -q "Generated by:"; then
        echo "❌ AI commits require 'Generated by:' section"
        exit 1
    fi
    if ! echo "$BODY" | grep -q "Validated by:"; then
        echo "❌ AI commits require 'Validated by:' section"
        exit 1
    fi
fi
...
echo "🧠 Running AI-assisted review..."
...
echo "✅ AI review passed. Ready to push!"
EOF
