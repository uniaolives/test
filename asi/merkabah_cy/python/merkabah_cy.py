"""
Sistema MERKABAH-CY - Módulo Python
...
class CYRLAgent:
    ...
    def select_action(self, state: CYGeometry) -> Tuple[np.ndarray, float]:
        ...
        # Converte estado para grafo (extrai diagonal triple)
        h11 = state.h11
        diag_indices = np.arange(h11)
        diag_features = state.intersection_matrix[diag_indices, diag_indices, diag_indices]

        x = torch.tensor(diag_features, dtype=torch.float32)
        edge_index = self._build_edge_index(h11)
...
class CYTransformer(nn.Module):
    ...
    def _reconstruct_metric(self, params: torch.Tensor, dim: int) -> np.ndarray:
        ...
        base = params[:dim*dim].reshape(dim, dim).numpy()
    ...
    def _generate_intersection_matrix(self, h11: int) -> np.ndarray:
        ...
        return np.random.randint(-10, 10, size=(h11, h11, h11))
...
class EntityEmergenceSimulator:
    ...
    def __init__(self, cy_transformer: CYTransformer):
        ...
        self.beta_range = np.linspace(0.1, 10.0, 100)  # Temperaturas inversas

    def simulate_phase_transition(self, z_base: torch.Tensor, steps: int = 1000):
