...
class MorphogeneticField:
    ...
    def __init__(self, shape: tuple = (100, 100, 100), diffusion_rates: tuple = (0.1, 0.05)):
        ...
        # Agent positions mapped to field coordinates
    ...
    def query_gradient(self, position: tuple) -> cp.ndarray:
        """
        Sample morphogen gradient at position (agent perception).
        """
        x, y, z = int(position[0]) % self.shape[0], int(position[1]) % self.shape[1], int(position[2]) % self.shape[2]

        # Compute local gradient
        grad_x = (self.A[(x+1)%self.shape[0], y, z] - self.A[(x-1)%self.shape[0], y, z]) / 2
        grad_y = (self.A[x, (y+1)%self.shape[1], z] - self.A[x, (y-1)%self.shape[1], z]) / 2
        grad_z = (self.A[x, y, (z+1)%self.shape[2]] - self.A[x, y, (z-1)%self.shape[2]]) / 2
