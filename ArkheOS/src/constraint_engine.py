...
try:
    from .bio_arkhe import ArkheGenome
except ImportError:
    try:
        from bio_arkhe import ArkheGenome
    except ImportError:
        # Fallback se não encontrar bio_arkhe (apenas para evitar erro de importação se não usado)
        ArkheGenome = None
...
class SynapticTrace:
    ...
    def decay_factor(self, current_time: float, tau: float = 100.0) -> float:
        ...
        return float(np.exp(-abs(current_time - self.timestamp) / tau))
...
class ConstraintLearner:
    ...
    def __init__(self, agent_id: int, genome_vector: Optional[np.ndarray] = None):
        ...
        self.working_memory: deque = deque(maxlen=15)
    ...
    def evaluate_partner(self, partner_genome, current_time: float = 0.0) -> Tuple[float, str]:
        ...
        # 4. Modulação por curiosidade
        uncertainty = 1.0 - min(1.0, float(np.mean(np.abs(self.weights))) * 2)
    ...
    def get_cognitive_profile(self) -> str:
        ...
        if total < 5: return "Neófito"
        success_rate = self.metrics['successful_interactions'] / total
        if success_rate > 0.75: return "Especialista"
        elif success_rate > 0.45: return "Aprendiz"
        elif success_rate > 0.25: return "Explorador"
        else: return "Cauteloso"

    def get_preferences(self) -> str:
        labels = ["Química", "Informação", "Energia", "Função"]
        max_idx = np.argmax(np.abs(self.weights))
        if abs(self.weights[max_idx]) < 0.15: return "Explorando padrões"
