...
class BioGenesisEngine:
    ...
    def _initialize_population(self, num_agents: int) -> None:
        ...
        """
        tribe_centers = [
            np.array([25.0, 25.0, 50.0]),
            np.array([50.0, 75.0, 50.0]),
            np.array([75.0, 50.0, 25.0])
        ]
        ...
        for i in range(num_agents):
            ...
            genome = ArkheGenome(
                ...
                I=float(np.random.uniform(0.2, 0.8)),
                E=float(np.random.uniform(0.4, 1.0)),
    ...
    def update(self, dt: float = 0.1) -> None:
        ...
        # 4. Processa interações sociais
        self._process_interactions()

        # 5. Aprendizado Hebbiano
    ...
    def inject_signal(self, x: float, y: float, z: float,
        ...
        """API para injetar sinal externo."""
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            'agents': len(alive),
            'time': round(float(self.simulation_time), 1),
            'bonds': int(self.stats['bonds_formed']),
            'deaths': int(self.stats['deaths']),
            'avg_health': round(float(np.mean([a.health for a in alive])), 3) if alive else 0.0
        }
    ...
    def get_agent_info(self, agent_id: int) -> Optional[Dict[str, Any]]:
        ...
        info = {
            ...
            'genome': {
                'C': round(float(agent.genome.C), 2),
                'I': round(float(agent.genome.I), 2),
                'E': round(float(agent.genome.E), 2),
                'F': round(float(agent.genome.F), 2)
            },
