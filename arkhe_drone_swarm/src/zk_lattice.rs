...
pub const Q_MODULUS: i64 = 3329; // Primo para o anel de polinómios (ex: Kyber)
pub const N_DEGREE: usize = 256; // Grau do polinómio
...
impl Polynomial {
    ...
    pub fn add(&self, other: &Self) -> Self {
        ...
        for i in 0..N_DEGREE {
            result[i] = (self.coeffs[i] + other.coeffs[i]).rem_euclid(Q_MODULUS);
        }
    ...
    pub fn multiply(&self, other: &Self) -> Self {
        ...
        for i in 0..N_DEGREE {
            for j in 0..N_DEGREE {
                ...
                result[index] = (result[index] + sign * self.coeffs[i] * other.coeffs[j]).rem_euclid(Q_MODULUS);
            }
        }
        Polynomial { coeffs: result }
    }
...
pub struct ArkheZKProver {
    public_matrix_a: Polynomial,
    secret_key_s: Polynomial,
}
