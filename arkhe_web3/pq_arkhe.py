...
class QuantumSafeChannel:
    ...
    def respond_handshake(self, init_msg: dict) -> dict:
        ...
        ct, self.shared_secret = Kyber512.encapsulate(peer_pk)
    ...
    def complete_handshake(self, resp_msg: dict) -> bool:
        ...
        self.shared_secret = Kyber512.decapsulate(ct, self.kyber_sk)

        # Calcular latência de volta
        t_roundtrip = time.time() - resp_msg['timestamp']
    ...
    def send_message(self, plaintext: bytes) -> dict:
        ...
        # Criptografar com shared_secret.
        # PoC: Em uma implementação de produção, usaríamos uma construção AEAD (ex: AES-GCM)
        # com a chave derivada do Kyber Shared Secret usando um KDF (ex: HKDF-SHA256).
    ...
    def _check_anomalies(self, current_latency: float, msg: dict) -> dict:
        ...
        if hasattr(self, '_last_jitter'):
            # np.corrcoef can return NaN if input is constant
            with np.errstate(invalid='ignore'):
                corr_matrix = np.corrcoef(current_jitter, self._last_jitter)
                jitter_corr = corr_matrix[0,1] if not np.isnan(corr_matrix[0,1]) else 1.0
        ...
        if hasattr(self, '_baseline_entropy') and self._baseline_entropy > 0:
