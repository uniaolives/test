...
class IsingConsensusEngine:
    ...
    def _validar_proposta(self, validador_id: str, proposta: bytes) -> float:
        ...
        """
        # Simulação determinística baseada no conteúdo
        import hashlib
        seed_data = proposta + validador_id.encode()
        seed = int(hashlib.sha256(seed_data).hexdigest(), 16) % 1000

        # Garante reprodutibilidade local
        state = np.random.get_state()
        np.random.seed(seed)
        val = (np.random.random() * 2.0) - 1.0
    ...
    def alcancar_consensus(self, proposta: bytes, max_iter: int = 1000,
        ...
        for t in range(max_iter):
            ...
            h = self._calcular_campos_locais(proposta)
            historico_E.append(self._energia_total(h))
