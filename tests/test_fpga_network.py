...
def test_fpga_emulator_noise():
    ...
    # In our model, noise reduces amplitudes, so coherence (proxy) should drop
    # but renormalization keeps it at 1.0 for pure state vectors.
    # We test the renormalization logic.
    assert np.isclose(np.linalg.norm(emulator.state), 1.0)
    assert final_coherence == 1.0 # Pure state vector always 1.0
